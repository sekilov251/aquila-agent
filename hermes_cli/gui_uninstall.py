...
def desktop_userdata_dir() -> Path:
    ...
    "Aquila Agent" on each platform. This is GUI-only state (connection.json,
    updates.json, Chromium cache) and never holds agent config or sessions.
    """
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Aquila Agent"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "Aquila Agent"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Aquila Agent.app"),
            home / "Applications" / "Aquila Agent.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Aquila Agent).
            local_base / "Programs" / "Aquila Agent",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Aquila Agent")
    else:
