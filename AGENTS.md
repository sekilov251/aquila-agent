# Aquila Agent - Development Guide
...
Aquila is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Aquila ships a **lot** — most merges are bug fixes to
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.hermes/plugins/` (or via a
  pip entry point), and promote them in the Savez Discord
...
├── cli.py                # AquilaCLI class — interactive CLI orchestrator (~11k LOC)
...
Applies to TypeScript across Aquila: desktop, TUI, website, and future TS packages.
...
- `process_command()` is a method on `AquilaCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
...
```
2. Add handler in `AquilaCLI.process_command()` in `cli.py`:
```python
...
be core tools. For custom or local-only tools, do **not** edit Aquila core.
...
core Aquila tool that should ship in the base system.
...
- `default` — Classic Aquila gold/kawaii (the current look)
...
Aquila has two plugin surfaces. Both live under `plugins/` in the repo so
...
don't own. Promote standalone plugins in the Savez Discord
...
2. **Tools referenced in SKILL.md prose must be native Aquila tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Aquila Agent" is the secondary collaborator. If the
   contributor's commit shows "Aquila Agent" as author (because they
   used Aquila to draft the skill), replace it with their actual name
...
Aquila returns a delegation id immediately and the result re-enters the
...
Aquila-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Aquila supports **profiles** — multiple fully isolated instances, each with its own
