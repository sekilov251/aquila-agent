# Aquila Observer Hooks

Aquila observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Aquila' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Aquila catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Aquila only builds
