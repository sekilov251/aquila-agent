 ...
 *
 *   macOS:   …/Aquila.app/Contents/MacOS/Aquila  → …/Aquila.app
 *   Windows: …\Aquila\Aquila.exe                 → …\Aquila  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Aquila.app/Contents/MacOS/Aquila → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Aquila.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Aquila.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Aquila$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Aquila install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
