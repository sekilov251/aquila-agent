...
import { buildDesktopBackendEnv, normalizeAquilaHomeRoot } from './backend-env'
import { canImportAquilaCli, verifyAquilaCli } from './backend-probes'
...
if (IS_WINDOWS) {
  ...
  // expose Aquila sessions/config to every packaged app on the machine.
...
// HERMES_HOME — the user-facing root for everything Aquila-related. Mirrors
// scripts/install.ps1's $AquilaHome and scripts/install.sh's $HERMES_HOME.
//
// Defaults:
...
function resolveAquilaHome() {
  if (process.env.HERMES_HOME) {
    return normalizeAquilaHomeRoot(process.env.HERMES_HOME)
  }
  ...
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) {
      return normalizeAquilaHomeRoot(fromRegistry)
    }
  }
...
const HERMES_HOME = resolveAquilaHome()
...
function pathWithAquilaManagedNode(...entries) {
  return [...hermesManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_HERMES_ROOT — the canonical mutable Aquila install. Same path
...
// boots, going straight to `resolveAquilaBackend()`. Missing or stale marker
...
// active-profile.json records which Aquila profile the desktop launches its
// local backend as. When set, startAquila() passes `hermes --profile <name>
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Aquila'
...
// electron-builder derives from the build `appId` (com.savez.aquila) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.savez.aquila')
}

// Seed the native About panel with the live Aquila version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `hermes update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAquilaVersion(),
  copyright: 'Copyright © 2026 Savez'
})
...
// startAquila(); this pool only holds EXTRA profile
...
// onto the error so subsequent startAquila() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Aquila backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Aquila will start automatically when it completes…',
      12
    )
...
function unwrapWindowsVenvAquilaCommand(command, backendArgs) {
  return resolveVenvAquilaCommand(command, backendArgs, {
...
function isAquilaSourceRoot(root) {
...
function findSystemPython() {
  ...
  // CPython but several Aquila deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Aquila' terminal tool requires
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    !IS_PACKAGED && isAquilaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAquilaSourceRoot(ACTIVE_HERMES_ROOT) ? ACTIVE_HERMES_ROOT : null
  ].filter(Boolean)
...
function venvAquilaShimPath(updateRoot) {
  return IS_WINDOWS
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const shim = venvAquilaShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentAquilaProcess = backendConnectionState.getProcess()

    if (currentAquilaProcess && Number.isInteger(currentAquilaProcess.pid)) {
      stragglers.push(currentAquilaProcess.pid)
    }
...
// `hermes update`). Instead we spawn the staged Aquila-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Aquila — this window will close and the updater will open. Don’t reopen Aquila yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Aquila install open ' +
        '(a second Aquila window or a terminal running hermes?). Close it and retry.'
    ...
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        HERMES_HOME,
        PATH: pathWithAquilaManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAquila = path.join(venvBin, IS_WINDOWS ? 'hermes.exe' : 'hermes')
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvAquila) || fileExists(path.join(updateRoot, '.hermes-bootstrap-complete'))
...
function resolveAquilaCliBinary(updateRoot) {
  const venvAquila = path.join(updateRoot, 'venv', 'bin', 'hermes')

  if (fileExists(venvAquila)) {
    return venvAquila
  }
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const hermes = resolveAquilaCliBinary(updateRoot)
  ...
  // Put the Aquila-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  const env: Record<string, string> = {
    HERMES_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithAquilaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Aquila (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Aquila to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Aquila…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Aquila to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Aquila desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Aquila to finish.'
    }
  }
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Aquila to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Aquila to load the new version.',
      percent: 100
    })
...
function isActiveRuntimeUsable() {
  ...
  return (
    isAquilaSourceRoot(ACTIVE_HERMES_ROOT) &&
    fileExists(venvPython) &&
    canImportAquilaCli(venvPython, {
      env: {
...
function resolveAquilaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Aquila.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAquilaCwd(), sanitized: Boolean(trimmed) }
  }
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Aquila at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveAquilaBackend(backendArgs) {
  // 1. Explicit override -- HERMES_DESKTOP_HERMES_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HERMES_DESKTOP_HERMES_ROOT && path.resolve(process.env.HERMES_DESKTOP_HERMES_ROOT)

  if (overrideRoot && isAquilaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Aquila source at ${overrideRoot}`, backendArgs)
  ...
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAquilaSourceRoot.)
  if (!IS_PACKAGED && isAquilaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Aquila source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Aquila override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Aquila CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }

    if (hermesCommand) {
      const unwrapped = unwrapWindowsVenvAquilaCommand(hermesCommand, backendArgs)
      ...
      if (verifyAquilaCli(hermesCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvAquilaCommand(hermesCommand, backendArgs) || {
            label: `existing Aquila CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Aquila CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  //    resolveAquilaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Aquila Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAquilaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Aquila install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Aquila recovery was handed off to Aquila Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Aquila install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Aquila bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
      ...
      // Latch the failure so subsequent startAquila() calls return this
    ...
    return ensureRuntime(resolveAquilaBackend(backend.args))
  }
  ...
  if (!isAquilaSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Aquila install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Aquila' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Aquila on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Aquila.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAquilaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Aquila venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Aquila at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Aquila runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Aquila backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Aquila backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Aquila backend after ${timeoutMs}ms`))
    })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAquilaCwd()
...
async function waitForAquila(baseUrl, token) {
  ...
  throw new Error(`Aquila backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting Aquila...".
...
//
// Hosted Aquila gateways gate the dashboard behind an OAuth provider (e.g.
// Savez) instead of a static session token. The auth model is
...
// "not signed in", which on the initial boot path (startAquila → the renderer's
// single-shot boot() with no retry) surfaced as the "Aquila couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Aquila
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Aquila Cloud agent…' : 'Sign in to Aquila gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Aquila backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Aquila Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// so a single override flips every Aquila surface to the same portal.
...
// authenticates via PRIVY, not the Aquila gateway session cookies, so this
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Aquila Cloud sign-in.'))
...
// Discover the hosted (Aquila Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Aquila Cloud. Open Settings → Gateway, choose Aquila Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Aquila Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Aquila Cloud session has expired. Sign in to Aquila Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Aquila Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  // saved mode, preserving 'cloud' (a Aquila Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Aquila Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Aquila Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Aquila Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Aquila gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Aquila gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Aquila backend.'
      )
    }
...
// env → global) so a startAquila() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Aquila
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Savez" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAquila())
  ...
  // connect to Aquila gateway". Mirror the renderer's connect here so the test
...
function resetAquilaConnection({ soft = false } = {}) {
...
// startAquila() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetAquilaConnection() invalidates it.
  const hermesProcess = backendConnectionState.getProcess()
...
// profile to startAquila() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAquila() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAquila(remote.baseUrl, remote.token)
  ...
  const backend = await ensureRuntime(resolveAquilaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const hermesCwd = resolveAquilaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Aquila backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Aquila backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Aquila backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Aquila backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForAquila(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Aquila backend for profile "${profile}"`,
    rememberLog
  })
...
async function startAquila() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAquila() call re-throws the same error
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Aquila backend', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Aquila backend at ${remote.baseUrl}`, 24)
      await waitForAquila(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Aquila backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Aquila runtime', 28)
    const backend = await ensureRuntime(resolveAquilaBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const hermesCwd = resolveAquilaCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Aquila backend via ${backend.label}`, 84)
    rememberLog(`Starting Aquila backend via ${backend.label}`)
    ...
    const hermesProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...
          // resolves to the SAME location our resolveAquilaHome() picked. Without
    ...
    if (!processOwner) {
      stopBackendChild(hermesProcess)
      throw new Error('Aquila backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Aquila backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Aquila backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Aquila backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Aquila backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Aquila backend exit (${signal || code})`)
      ...
      rememberLog(`Aquila backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Aquila backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Aquila backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAquilaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Aquila backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Aquila backend to become ready', 90)
    await Promise.race([waitForAquila(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Aquila backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Aquila is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Aquila app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Aquila icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Aquila in the normal
...
function createWindow() {
  ...
  startAquila().catch(error => rememberLog(error.stack || error.message))
...
// "Starting Aquila…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAquilaConnection only
    // clears the connection promise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Aquila backend failed liveness probe; dropping stale connection.')
    resetAquilaConnection()
...
ipcMain.handle('hermes:bootstrap:reset', async () => {
  ...
  // reset connection state so the next startAquila() call restarts the
...
ipcMain.handle('hermes:bootstrap:repair', async () => {
  ...
  // startAquila() re-runs the full installer (refreshing a broken/partial
...
// --- Aquila Cloud (cloud-auto-discovery Phase 3) ---
...
// it via writeDefaultProjectDir so resolveAquilaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('hermes:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAquilaCwd()
}))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Aquila' TUI into
...
// Resolve the canonical Aquila version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Aquila version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAquilaVersion() {
  try {
...
// Re-resolve the live Aquila version and push it into the native About panel
...
ipcMain.handle('hermes:version', async () => ({
  appVersion: resolveAquilaVersion(),
...
async function getUninstallSummary() {
  ...
  const fallback = () => ({
    ...
    agent_installed: isAquilaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Aquila agent venv at ${VENV_ROOT}.`
    }
  }
...
// ---------------------------------------------------------------------------
// aquila:// deep links (e.g. aquila://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const HERMES_PROTOCOL = 'aquila'
...
function handleDeepLink(url) {
  ...
  // aquila://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
...
// second-instance argv. Without the lock a second `aquila://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching aquila:// URL is in our own argv.
