 ...
 * unwrapWindowsVenvAquilaCommand(). Each of the three functions here pins one
 ...
 *   3. resolveVenvAquilaCommand() — unwrapWindowsVenvAquilaCommand() returned
...
export interface ResolveVenvAquilaCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportAquilaCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
 ...
 * ONLY after smoke-testing that interpreter with canImportAquilaCli(). A
 ...
 */
export function resolveVenvAquilaCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvAquilaCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Aquila at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Aquila Python at ${python}`,
    command: python,
