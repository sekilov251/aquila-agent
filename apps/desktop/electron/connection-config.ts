 ...
 *     `X-Hermes-Session-Token` header (legacy wire name); WS uses `?token=`.
...
// The Nous portal (NAS) does NOT use Aquila gateway session cookies — it is a
...
// Aquila Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
 ...
 * the Aquila gateway session cookies, so cloud sign-in / discovery liveness
