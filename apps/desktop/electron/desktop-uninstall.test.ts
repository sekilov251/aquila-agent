...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Aquila.app/Contents/MacOS/Aquila', 'darwin'),
    '/Applications/Aquila.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Aquila.app/Contents/MacOS/Aquila', 'darwin'),
    '/Users/x/Applications/Aquila.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Aquila\\Aquila.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Aquila'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Aquila.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Aquila.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_AquilaXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Aquila.AppImage' }),
    '/home/x/Apps/Aquila.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Aquila.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Aquila.app'), false)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Aquila',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Aquila" >nul 2>&1/)
