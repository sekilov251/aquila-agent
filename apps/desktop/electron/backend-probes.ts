 ...
 * resolveAquilaBackend (main.ts). The resolver walks a ladder of
 ...
 *     not surfaced to the user, just to recentAquilaLog for forensics
...
/**
 * Return the Python snippet used to verify Aquila can import far enough to
...
/**
 * Return true iff the Aquila runtime import probe exits 0.
 *
 * Used to gate the "fallback to system Python with hermes_cli installed"
 * rung of resolveAquilaBackend. Without this, a system Python 3.11-3.13
 ...
 * @returns {boolean}
 */
function canImportAquilaCli(pythonPath: string, opts: { env?: Record<string, string> } = {}) {
  if (!pythonPath) {
    return false
  }
 ...
 *   in resolveAquilaBackend.
 * @returns {boolean}
 */
function verifyAquilaCli(hermesCommand: string, opts?: { shell?: boolean }) {
  if (!hermesCommand) {
    return false
  }
...
export { canImportAquilaCli, hermesRuntimeImportProbe, PROBE_TIMEOUT_MS, verifyAquilaCli }
