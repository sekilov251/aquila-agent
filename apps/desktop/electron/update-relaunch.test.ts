...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Aquila12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/hermes/hermes', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Aquila/hermes', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'aquila://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'aquila://open/agent/42', '--profile=work'])
...
test('collectRelaunchEnv preserves HERMES_HOME + HERMES_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    ...
    HERMES_DESKTOP_APP_NAME: 'AquilaSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.hermes/hermes-agent/apps/desktop/release/linux-unpacked/Aquila',
    args: ['aquila://open/agent/42', "--note=it's fine"],
  ...
  assert.match(script, /exec '.*\/linux-unpacked\/Aquila' 'hermes:\/\/open\/agent\/42' '--note=it'\\''s fine'/)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  assert.match(script, /exec '\/opt\/Aquila\/Aquila'\n/)
})
