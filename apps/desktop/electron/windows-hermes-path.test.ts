...
// unwrapWindowsVenvAquilaCommand(). These pin the two Windows resolution bugs
...
//   3. resolveVenvAquilaCommand() — must probe the venv python via
//      canImportAquilaCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvAquilaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportAquilaCli: () => true,
...
test('resolveVenvAquilaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvAquilaCommand('/root/venv/Scripts/hermes.exe', [], deps), null)
})

test('resolveVenvAquilaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvAquilaCommand('/root/venv/Scripts/hermes.cmd', [], deps), null)
})

test('resolveVenvAquilaCommand: returns null when the basename is not hermes/hermes.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAquilaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvAquilaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAquilaCommand('/root/venv/bin/hermes.exe', [], deps), null)
})

test('resolveVenvAquilaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvAquilaCommand('/root/venv/Scripts/hermes.exe', [], deps), null)
})

test('resolveVenvAquilaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportAquilaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvAquilaCommand('/root/venv/Scripts/hermes.exe', ['serve'], deps)
...
test('resolveVenvAquilaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvAquilaCommand('/root/venv/Scripts/hermes.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvAquilaCommand: is case-insensitive on hermes.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvAquilaCommand('/root/venv/Scripts/HERMES.EXE', [], deps))
  assert.ok(resolveVenvAquilaCommand('/root/venv/SCRIPTS/hermes.exe', [], deps))
})
