#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Aquila icon + version metadata onto the
// built Aquila.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Aquila.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Aquila.exe>
//
...
// Stamp the Aquila icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Aquila',
      FileDescription: 'Aquila',
      CompanyName: 'Savez',
      LegalCopyright: 'Copyright (c) 2026 Savez'
    }
  })

  console.log('[set-exe-identity] done — Aquila icon + identity stamped')
}
