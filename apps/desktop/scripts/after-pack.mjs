 ...
 *
 * Stamps the Aquila icon + identity onto the packed Windows Aquila.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Aquila')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Aquila'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Aquila.exe keeps the stock Electron icon`)
  }
}
