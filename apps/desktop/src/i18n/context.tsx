...
import { getAquilaConfigRecord, type AquilaConfigRecord, saveAquilaConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AquilaConfigRecord>
  saveConfig: (config: AquilaConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  ...
  saveConfig: config => {
    ...
    return saveAquilaConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: AquilaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AquilaConfigRecord, locale: Locale): AquilaConfigRecord {
