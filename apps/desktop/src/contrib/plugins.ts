 ...
 *    `AquilaPlugin` registers automatically (vite glob — drop a folder in).
...
import { createPluginContext, type AquilaPlugin } from './plugin'
import { pluginActive, publishPlugin } from './plugins-store'
import { watchRuntimePlugins } from './runtime-loader'

const modules = import.meta.glob<{ default: AquilaPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default AquilaPlugin export — skipped`)
