...
/** Why a known Aquila command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Aquila profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Aquila Agent version', surface: exec() },
...
function isKnownAquilaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Aquila' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAquilaSlashCommand(normalized)
}
