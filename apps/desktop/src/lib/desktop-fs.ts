import type {
  AquilaConnection,
  AquilaReadDirResult,
  AquilaReadFileTextResult,
  AquilaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: AquilaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Aquila Desktop bridge is unavailable')
  }
...
export async function readDesktopDir(path: string): Promise<AquilaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<AquilaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<AquilaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<AquilaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: AquilaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
