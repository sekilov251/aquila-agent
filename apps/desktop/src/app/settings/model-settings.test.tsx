...
const getAquilaConfigRecord = vi.fn()
const saveAquilaConfig = vi.fn()
...
vi.mock('@/hermes', () => ({
  ...
  getAquilaConfigRecord: () => getAquilaConfigRecord(),
  saveAquilaConfig: (config: unknown) => saveAquilaConfig(config),
...
beforeEach(() => {
  ...
  getAquilaConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveAquilaConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getAquilaConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveAquilaConfig).toHaveBeenCalledWith(
