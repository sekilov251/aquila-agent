...
import { invalidateAquilaConfig, setAquilaConfigCache, useAquilaConfigRecord } from '../hooks/use-config-record'
...
// Aquila' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Aquila default (medium), shown as Medium.
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useAquilaConfigRecord()
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Aquila runs the flow for you.`}
          </p>
        )}
