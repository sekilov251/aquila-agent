...
import type { AquilaGateway } from '@/hermes'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<AquilaGateway | null>(null)
  ...
  const reconnectingRef = useRef<Promise<AquilaGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AquilaGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('Aquila gateway unavailable')
      }
