import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@aquila/shared'
import { useEffect, useRef } from 'react'

import type { AquilaConnection } from '@/global'
import { AquilaGateway } from '@/hermes'
...
interface GatewayBootOptions {
  ...
  onGatewayReady: (gateway: AquilaGateway | null) => void
  refreshAquilaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AquilaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Aquila...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Aquila…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Aquila gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAquilaConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshAquilaConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startAquila re-emits progress — ignore so the
    ...
    const gateway = new AquilaGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshAquilaConfig(),
