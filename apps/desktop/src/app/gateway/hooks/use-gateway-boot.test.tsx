...
// bug that drives the REAL useGatewayBoot hook + REAL AquilaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAquilaConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAquila) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAquila()'s remote branch awaits waitForAquila() for 45s before it
    ...
    // After ~45s waitForAquila gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Aquila backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
