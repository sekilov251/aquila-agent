...
import type { AquilaGitBranch } from '@/global'
...
const branchActionLabel = (branch: AquilaGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<AquilaGitBranch[]>([])
  ...
  const convert = async (branch: AquilaGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
