...
import { getAquilaConfig } from '@/hermes'
...
import { useAquilaConfig } from './use-hermes-config'

vi.mock('@/hermes', () => ({
  getAquilaConfig: vi.fn(),
  getAquilaConfigDefaults: vi.fn().mockResolvedValue({})
}))
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getAquilaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getAquilaConfig>>)

describe('useAquilaConfig refreshAquilaConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshAquilaConfig()
    })
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getAquilaConfig>>>()
    vi.mocked(getAquilaConfig).mockReturnValueOnce(profileConfig.promise)
    ...
    act(() => {
      pendingRefresh = result.current.refreshAquilaConfig(true)
    })
    expect(getAquilaConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getAquilaConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getAquilaConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getAquilaConfig>>>()
    vi.mocked(getAquilaConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshAquilaConfig(true)
      refreshC = result.current.refreshAquilaConfig(true)
    })
