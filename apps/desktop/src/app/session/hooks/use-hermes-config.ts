...
import { getAquilaConfig, getAquilaConfigDefaults } from '@/hermes'
...
interface AquilaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAquilaConfig({ activeSessionIdRef, refreshProjectBranch }: AquilaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAquilaConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getAquilaConfig(), getAquilaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAquilaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
