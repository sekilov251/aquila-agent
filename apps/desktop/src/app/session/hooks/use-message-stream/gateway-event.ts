...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshAquilaConfig — two REST calls (config + defaults) per event, per
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (apply) {
          ...
          // everything refreshAquilaConfig applies is either active-session
        ...
        const errorMessage = payload?.message || 'Aquila reported an error'
