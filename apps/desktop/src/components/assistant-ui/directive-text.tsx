...
type AquilaRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<AquilaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AquilaRefType] ?? ICON_FALLBACK
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AquilaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AquilaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Aquila directives (`@file:...`, `@image:...`) as
