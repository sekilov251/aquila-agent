...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AquilaConnection>
      ...
      // Aquila Cloud: one portal login powers discovery + silent per-agent
      ...
      api: <T>(request: AquilaApiRequest) => Promise<T>
      notify: (payload: AquilaNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<AquilaReadFileTextResult>
      selectPaths: (options?: AquilaSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AquilaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AquilaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: AquilaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AquilaReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<AquilaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<AquilaGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<AquilaGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<AquilaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AquilaReviewScope, baseRef?: null | string) => Promise<AquilaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AquilaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AquilaReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: AquilaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AquilaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AquilaWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: AquilaPreviewFileChanged) => void) => () => void
...
export interface AquilaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AquilaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface AquilaConnection {
  baseUrl: string
...
export interface AquilaTitleBarTheme {
  background: string
  foreground: string
}

export interface AquilaWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Aquila Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Aquila Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Aquila Cloud org (slug or id) to
...
// A discovered Aquila Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface AquilaApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface AquilaNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface AquilaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AquilaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AquilaPreviewWatch {
  id: string
  path: string
}
...
export interface AquilaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface AquilaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface AquilaGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface AquilaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface AquilaRepoStatus {
  branch: null | string
...
// turn began.
export type AquilaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AquilaReviewFile {
  path: string
  added: number
  removed: number
...
export interface AquilaReviewList {
  files: AquilaReviewFile[]
...
export interface AquilaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface AquilaReviewShipInfo {
  ghReady: boolean
  pr: AquilaReviewPr | null
}

export interface AquilaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AquilaReadDirResult {
  entries: AquilaReadDirEntry[]
  error?: string
}

export interface AquilaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AquilaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
