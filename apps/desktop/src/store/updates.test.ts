...
const checkAquilaUpdateSpy = vi.fn()
const updateAquilaSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/hermes', () => ({
  checkAquilaUpdate: (...args: unknown[]) => checkAquilaUpdateSpy(...args),
  updateAquila: (...args: unknown[]) => updateAquilaSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkAquilaUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkAquilaUpdateSpy.mockResolvedValue({
      install_method: 'git',
    ...
    expect(checkAquilaUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkAquilaUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyUpdates terminal state', () => {
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Aquila to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateAquilaSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateAquilaSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkAquilaUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
