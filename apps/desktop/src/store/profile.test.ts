...
const remoteConn = (over: Partial<AquilaConnection> = {}): AquilaConnection =>
  ({ baseUrl: 'https://hermes-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AquilaConnection

const localConn = (over: Partial<AquilaConnection> = {}): AquilaConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AquilaConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AquilaConnection>>()
