...
import type { AquilaReviewFile, AquilaReviewShipInfo } from '@/global'
...
function file(path: string, over: Partial<AquilaReviewFile> = {}): AquilaReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as AquilaReviewFile
}
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as AquilaReviewShipInfo)
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: AquilaReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as AquilaReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { hermesDesktop?: unknown }).hermesDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as AquilaReviewShipInfo)
