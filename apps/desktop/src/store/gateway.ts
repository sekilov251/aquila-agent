import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@aquila/shared'
...
const isOpen = (gateway: AquilaGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<AquilaGateway | null>(null)
...
let primaryGateway: AquilaGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: AquilaGateway | null, profile = 'default'): void {
...
export function activeGateway(): AquilaGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<AquilaGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
