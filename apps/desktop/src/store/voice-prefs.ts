...
import { getAquilaConfigRecord, saveAquilaConfig } from '@/hermes'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getAquilaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveAquilaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
