...
import { checkAquilaUpdate, getActionStatus, updateAquila } from '@/hermes'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateAquila'),
      onClick: () => {
        snoozeSkewToast()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkAquilaUpdate(true))
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updateAquila()
