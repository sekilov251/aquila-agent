...
function stubProbe(impl: (cwd: string) => Promise<AquilaRepoStatus | null>) {
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: AquilaRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
