...
import type { AquilaGitBaseBranch, AquilaGitBranch } from '@/global'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Aquila gateway is not connected')
  }
...
// repo, returning where Aquila should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<AquilaGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<AquilaGitBaseBranch[]> {
  const git = desktopGit()
