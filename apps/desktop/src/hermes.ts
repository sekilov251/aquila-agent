import { JsonRpcGatewayClient } from '@aquila/shared'
...
// "Timed out connecting to Aquila backend" that hangs the UI (#48504).
//
...
export class AquilaGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Aquila gateway connection closed',
      connectErrorMessage: 'Could not connect to Aquila gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Aquila gateway is not connected',
...
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see AquilaApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('Aquila desktop bridge unavailable')
  }
...
export function getAquilaConfig(): Promise<AquilaConfig> {
  return window.hermesDesktop.api<AquilaConfig>({
    ...profileScoped(),
    path: '/api/config',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
...
export function getAquilaConfigRecord(): Promise<AquilaConfigRecord> {
  return window.hermesDesktop.api<AquilaConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getAquilaConfigDefaults(): Promise<AquilaConfigRecord> {
  return window.hermesDesktop.api<AquilaConfigRecord>({
    ...profileScoped(),
...
export function getAquilaConfigSchema(): Promise<ConfigSchemaResponse> {
...
export function saveAquilaConfig(config: AquilaConfigRecord): Promise<{ ok: boolean }> {
  return window.hermesDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
 ...
 *  `saveAquilaConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function updateAquila(): Promise<ActionResponse> {
  return window.hermesDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function checkAquilaUpdate(force = false): Promise<BackendUpdateCheckResponse> {
...
// config.yaml via saveAquilaConfig.
// ---------------------------------------------------------------------------
