...
//! Electron app's APP_ROOT/../.. trick, because Aquila-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Aquila-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/sekilov251/aquila-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
