...
/// Spawn the locally-built Aquila desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Aquila desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching Aquila desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Aquila
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Aquila.exe"),
            ("win-arm64-unpacked", "Aquila.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Aquila.app/Contents/MacOS", "Aquila"),
            ("mac-arm64/Aquila.app/Contents/MacOS", "Aquila"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Aquila.app/Contents/MacOS/Aquila -> .../Aquila.app
...
/// path so a bare re-open just opens Aquila instead of re-running setup.
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Aquila desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $AquilaHome which Stage-Repository clones
    // the repo INTO at $AquilaHome\hermes-agent. Mirrors hermes_constants.
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Aquila"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Aquila.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Aquila.exe");
    ...
    // what the updater ditto's over /Applications/Aquila.app). A regression in
