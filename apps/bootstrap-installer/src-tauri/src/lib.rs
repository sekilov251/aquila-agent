...
/// desktop app handing off via `Aquila-Setup.exe --update`).
///
...
pub fn run() {
    ...
    // Aquila is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Aquila installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Aquila is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Aquila" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Aquila" shortcuts that install.ps1 creates, and a
        ...
        .expect("error while running Aquila Setup");
}
