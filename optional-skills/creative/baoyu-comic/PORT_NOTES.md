...
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Aquila infra |
...
| File operations | Generic instructions | Aquila file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Aquila repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
