...
Maps each role archetype to the Aquila skills it should `always_load` and the
...
## Aquila skills relevant to video production
...
These are **not** Aquila skills but external CLIs / APIs that profiles invoke.
...
## Built-in Aquila tools for media review

These are native Aquila tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Aquila profile (Claude) |
...
When a new Aquila-public video skill ships:
