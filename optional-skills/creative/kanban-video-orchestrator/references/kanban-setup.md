...
the actual `setup.sh` that creates the project workspace, configures Aquila
...
PyYAML must be installed in the user's Python (it ships with most Aquila
...
the Aquila `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
