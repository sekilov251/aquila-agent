...
Companion skill for the `unreal-engine` entry in the Aquila MCP catalog. The
...
Two halves, in this order: the editor side must be up before Aquila connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Aquila — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Aquila connects
...
2. Start the Aquila session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Aquila
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Aquila runs batched calls
...
- **Start order matters.** Editor + server up first, then the Aquila
...
- **Timeouts on long operations.** Aquila' per-call default is 120 s; asset
