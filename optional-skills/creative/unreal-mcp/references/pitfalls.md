...
### 1. Start order: editor first, Aquila second

Aquila probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Aquila session.
...
restart the editor, restart the Aquila session.
...
server fails to bind (Output Log) or Aquila' probe times out. Fix: change
...
### 5. GenerateClientConfig is not for Aquila

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Aquila' connection
lives in `~/.hermes/config.yaml` via `hermes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Aquila — just don't
mistake it for the Aquila setup step.
...
explicitly warns against overlapping calls. Aquila executes same-turn tool
...
### 9. Timeouts: Aquila gives up before Unreal does

Aquila' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Aquila runs elsewhere (SSH backend,
