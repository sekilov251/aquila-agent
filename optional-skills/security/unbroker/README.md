...
The design is **Aquila-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
Then start a new Aquila session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Aquila session:
...
- **PII handling.** Dossiers live under the Aquila home directory (`0600`, optionally
