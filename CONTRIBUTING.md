# Contributing to Aquila Agent

Thank you for contributing to Aquila Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Aquila to work everywhere.
...
Related: #38284 covers the agent-side analog — Aquila itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Aquila install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Savez Discord](https://discord.gg/NousResearch). Users can install it with `hermes skills install`.
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Aquila ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Aquila Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Savez Discord](https://discord.gg/NousResearch) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Aquila venv, wires the `hermes` command, stamps the
...
Use this only if you intentionally do not want Aquila' managed install layout
...
├── cli.py                    # AquilaCLI class — interactive TUI, prompt_toolkit integration
...
The user may skip setup and keep loading the skill. Aquila only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native Aquila tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Aquila Agent" is the secondary collaborator. If the contributor's commit shows "Aquila Agent" as author because they used Aquila to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Aquila tools (`web_extract`, `terminal`, `read_file`).
...
Aquila uses a data-driven skin system — no code changes needed to add a new skin.
...
Aquila runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Aquila has terminal access. Security matters.
...
- Include: OS, Python version, Aquila version (`hermes version`), full error traceback
