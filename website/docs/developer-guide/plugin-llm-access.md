...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Aquila already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Aquila provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Aquila provider
        # (must be one Aquila already knows about — same names as
        ...
        # plugin sends — Aquila does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Aquila' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Aquila subsystem:
