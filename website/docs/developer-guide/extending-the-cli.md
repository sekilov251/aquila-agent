...
description: "Build wrapper CLIs that extend the Aquila TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Aquila exposes protected extension hooks on `AquilaCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Aquila."""

from cli import AquilaCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(AquilaCLI):
...
Called after Aquila registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
