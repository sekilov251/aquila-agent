...
The ACP adapter wraps Aquila' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> Aquila `once`
- `allow_always` -> Aquila `always`
- reject options -> Aquila `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Aquila tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Aquila' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Aquila provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Aquila' interactive model/provider configuration before starting a normal ACP session.
