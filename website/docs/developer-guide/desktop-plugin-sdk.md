...
description: "Extend the native Aquila Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Aquila Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `AquilaPlugin`.
...
"Plugin" means several unrelated things across Aquila. This page is the **native
...
CLI/gateway plugins are documented at [Build a Aquila Plugin](/developer-guide/plugins).
...
Both take the same `AquilaPlugin` contract, appear in **Settings → Plugins**, and
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Aquila' }),
      jsx('div', {
...
A plugin default-exports a `AquilaPlugin`:

```ts
interface AquilaPlugin {
...
`dashboard/` subfolder of a regular Aquila plugin and declare it in a
`manifest.json`:
...
export a `AquilaPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
| Plugin contract | `AquilaPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
