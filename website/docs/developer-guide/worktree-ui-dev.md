...
Neither is read by Aquila itself — they're private to these helpers. The variables Aquila *does* read are covered in [Environment Variables](../reference/environment-variables.md).
...
htui() {
  local root
  root="$(_hermes_root)" || { echo "htui: not in a Aquila checkout" >&2; return 1; }
...
`HERMES_TUI_DIR` points Aquila at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `hermes --tui --dev` exits with an error. Run `unset HERMES_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_hermes_root)" || { echo "hgui: not in a Aquila checkout" >&2; return 1; }
...
```bash
# The enclosing worktree, verified as a real Aquila checkout.
_hermes_root() {
  local root
...
- [Environment Variables](../reference/environment-variables.md) — every `HERMES_*` variable Aquila reads
