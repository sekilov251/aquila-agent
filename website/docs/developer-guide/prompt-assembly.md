...
description: "How Aquila builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Aquila deliberately separates:
...
You are Aquila, an AI assistant created by Savez.
...
The platform hint (Layer 10 above) is the per-surface guidance Aquila
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | Aquila-native project config |
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Aquila concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Aquila already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Aquila.
...
- if you want to change how Aquila assembles prompts for everyone, change Python and treat it as a code contribution
