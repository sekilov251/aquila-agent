...
sidebar_position: 1
title: "Architecture"
description: "Aquila Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Aquila Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # AquilaCLI — interactive terminal UI (large file)
...
```text
User input → AquilaCLI.process_input()
...
Exposes Aquila as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
