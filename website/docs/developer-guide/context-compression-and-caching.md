...
Aquila Agent uses a dual compression system and Anthropic prompt caching to
...
Aquila has two separate compression layers that operate independently:
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Aquila raises the
...
thread context, so Aquila' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Aquila
  records the resulting compaction events (compression counters, session
  events). Set `hermes` to let Aquila' compression threshold initiate
  app-server compaction, or `off` to disable Aquila-initiated automatic
  compaction entirely (codex may still compact natively).

Aquila' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Aquila' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Aquila
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Aquila can avoid; user-facing docs for `/model`, fallback providers, and
