...
description: "How to create skills for Aquila Agent — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to Aquila Agent. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, Aquila prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. Aquila never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
When loaded, Aquila checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
Prefer stdlib Python, curl, and existing Aquila tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
**Installing a blueprint.** When you install a skill that carries a `blueprint:` block, Aquila registers it as a **suggested cron job** rather than scheduling it. Scheduling is **opt-in** — installing never silently creates a recurring job. You review and accept it via `/suggestions`:
...
Aquila can *propose* automations and let you accept them with one tap, instead of making you assemble cron jobs by hand. Every proposal flows through one surface — the `/suggestions` command — regardless of where it came from:
...
Aquila can now consume third-party skills from multiple external discovery models:
