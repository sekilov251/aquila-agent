...
sidebar_position: 2
title: "Installation"
description: "Install Aquila Agent on Linux, macOS, WSL2, native Windows, or Android via Termux"
---

# Installation

Get Aquila Agent up and running in under two minutes!
...
If you want to install & run Aquila Desktop after a command-line only install, simply run
```bash
hermes desktop
```
...
Aquila auto-detects whether it was installed via `pip`, the git installer, Homebrew, or NixOS, and `hermes update` prints the matching update command for that path. There's no env var to set — the detection is based on the install layout (Python site-packages, `~/.hermes/hermes-agent/`, Homebrew prefix, or Nix store path). `hermes doctor` also surfaces the detected method under its environment summary.
