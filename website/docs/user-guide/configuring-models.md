...
When you save via the dashboard, Aquila writes to `~/.hermes/config.yaml`:
...
`provider: auto` with `model: ''` tells Aquila to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
