...
files. It is **never persisted** to any `.env` by Aquila.
...
If you try to change a managed value, Aquila refuses and names the source:
...
Changes take effect on the next Aquila start (a malformed managed file is logged
...
- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Aquila as `root`), managed scope is advisory.
