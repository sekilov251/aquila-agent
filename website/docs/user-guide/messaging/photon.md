...
Connect Aquila to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Aquila — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Aquila runs it in a
...
2. **Finds or creates** the `Aquila Agent` project on your account.
...
Photon uses the same authorization model as every other Aquila
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Aquila replies with a pairing code. Approve it with:
...
By default Aquila responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Aquila` and
`@Aquila agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Aquila will reply.
...
assigned iMessage line Aquila uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Aquila sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Aquila wake words  | JSON list / comma / newline regex patterns for group mentions |
