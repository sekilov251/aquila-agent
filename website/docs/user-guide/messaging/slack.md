...
sidebar_position: 4
title: "Slack"
description: "Set up Aquila Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Aquila Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Aquila instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Aquila uses the modern
...
The fastest path is to paste a manifest Aquila generates for you. It
...
### Option A: From a Aquila-generated manifest (recommended)
...
4. Enter an app name (e.g., "Aquila Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Aquila can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Aquila configuration issue.
:::
...
Aquila uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Aquila now
...
Every Aquila command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Aquila command with its description.

Under the hood: Aquila ships with a generated Slack app manifest (see
...
New Slack apps use Slack's **Agent** messaging experience. Existing Aquila
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Aquila can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Aquila only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Aquila adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — Aquila routes it the same way as `/btw
...
Slack never delivers them to Aquila.

As a workaround, Aquila recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Aquila treats it identically to the slash form and replies in the same
thread.
...
Aquila falls back to a text prompt, the prompt instructs you to reply
...
Understanding how Aquila behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Aquila Agent what time is it?`). In channels, Aquila replies in a thread attached to that message. |
| **Threads** | If you @mention Aquila inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
name in the thread. By default Aquila sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Aquila is thinking…"`) —
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Aquila in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Aquila will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Aquila will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Aquila Agent`).

---

## Multi-Workspace Support

Aquila can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Aquila also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Aquila uses the correct workspace-specific client to respond.
...
Aquila supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Aquila Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Aquila Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Aquila now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Aquila Agent`)
...
- Audit who has access to your Aquila config directory
