...
Connect Aquila to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Aquila supports two WeCom integration modes:
...
3. Aquila decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in Aquila and the token the
