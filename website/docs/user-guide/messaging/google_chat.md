...
sidebar_position: 12
title: "Google Chat"
description: "Set up Aquila Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Aquila Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Aquila
...
download the file. Save it somewhere only Aquila can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Aquila calls
...
- **App name**: whatever you want users to see ("Aquila" is reasonable).
...
`ADDED_TO_SPACE` event that Aquila uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Aquila
...
Install the dependencies the Google Chat adapter needs (no Aquila extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Aquila is thinking…" marker, then
...
platforms:
  google_chat:
    # Custom working-state marker text (default: "Aquila is thinking…").
...
Thread support: when a user replies inside a thread, Aquila detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Aquila session.
...
3. Download the JSON. Move it onto the host that runs Aquila.
4. Register the client with Aquila (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Aquila home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Aquila isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Aquila will only attach the SA bearer
