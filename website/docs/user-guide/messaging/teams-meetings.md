...
Use the Teams meeting pipeline when you want Aquila to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Use this when you want Aquila to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Aquila normally after updating config:

```bash
hermes gateway run
```

Or, if you run Aquila in Docker, start the gateway the same way you already do for your deployment.
