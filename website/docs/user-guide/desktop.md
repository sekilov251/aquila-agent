...
sidebar_position: 3
title: "Desktop App"
description: "The native Aquila desktop app — a polished experience for chatting with Aquila, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Aquila desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Aquila Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Aquila has several front ends that all talk to the same agent:
...
Follow the [installation instructions for Aquila Desktop](../getting-started/installation.md).

If you already have Aquila installed, simply run
...
- **The same conversation history** as every other Aquila surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to Aquila and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader Aquila management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [Aquila profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Aquila agent, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the Aquila source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Savez](./features/web-dashboard.md#default-provider-nous-research).
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Aquila Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Aquila Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
