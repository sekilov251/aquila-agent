...
title: "Plugins"
description: "Extend Aquila with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Aquila has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Aquila
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Aquila Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.
...
Start Aquila — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal Aquila plugin — registers a tool and a hook."""
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart Aquila, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Aquila.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Aquila — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
Within each source, Aquila also recognizes sub-category directories that route plugins to specialized discovery systems:
...
Several categories of plugin bypass `plugins.enabled` — they're part of Aquila' built-in surface and would break basic functionality if gated off by default:
...
Aquila has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Aquila also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Aquila Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Aquila Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Aquila Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Aquila auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
