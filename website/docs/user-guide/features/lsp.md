...
Aquila runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Aquila
...
1. Aquila captures a baseline of current diagnostics for the file.
...
…). Aquila auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Aquila at the extracted bundle — the directory that contains
...
`node_modules` tree — Aquila installs both packages together when you
...
When `install_strategy: auto`, Aquila installs binaries into
...
shared location — the staging dir is fully Aquila-owned and is
...
Servers are kept alive for the life of the Aquila process. There's
