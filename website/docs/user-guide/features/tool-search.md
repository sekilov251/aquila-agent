...
**Tool Search** is Aquila' opt-in progressive-disclosure layer for that
...
:::info Built-in Aquila tools never defer
The tools that make up Aquila' core capability set (`terminal`,
...
When the model invokes `tool_call`, Aquila **unwraps the bridge** and
...
- **No JS sandbox.** Aquila uses the simpler "structured tools" mode
