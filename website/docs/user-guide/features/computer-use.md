...
Aquila Agent can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Aquila app). `hermes computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Aquila run declares its own cua-driver
...
Aquila intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Aquila-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Aquila skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Aquila
...
Aquila applies multi-layer guardrails:
...
Screenshots are expensive. Aquila applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Aquila agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Aquila agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Aquila warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Aquila from inside the RDP / console
...
upstream. **Aquila disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Aquila sets
...
unreleased fix — point Aquila at a binary you built from source instead
of the published release. Aquila resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Aquila testing (see notes).
...
### Option B — point Aquila straight at the built binary (fastest loop)
...
### Confirm Aquila is using your build
...
- **Aquila spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Aquila
  change needed either way.
- **Local builds skip the version check.** Aquila warns when the
...
- **Aquila-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Aquila `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Aquila (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Aquila among them)
