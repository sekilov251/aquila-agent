...
| Version | `1.0.0` |
| Author | Anthropic (adapted by Savez) |
| License | Apache-2.0 |
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Aquila:

- **If you have any structured financial-data MCP configured** (Aquila supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
