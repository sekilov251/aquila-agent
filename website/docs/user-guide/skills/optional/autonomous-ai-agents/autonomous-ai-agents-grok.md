...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Aquila Agent |
| License | MIT |
...
# Grok Build CLI — Aquila Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Aquila terminal. Grok
...
1. Prepare stable input files first with Aquila tools (`read_file`,
...
2. **Don't conflate Aquila' xAI auth with the `grok` CLI's auth.** Aquila'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Aquila' xAI auth carries
   over.
