...
- smoke-test a server with the FastMCP CLI before wiring it into Aquila or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Aquila. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Aquila integration, either:
...
### Aquila cannot see the deployed server

The server-building part may be correct while the Aquila config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Aquila.
