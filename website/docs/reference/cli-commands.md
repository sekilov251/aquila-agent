...
description: "Authoritative reference for Aquila terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Aquila profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes backup` | Back up Aquila home directory to a zip file. |
...
| `hermes import` | Restore a Aquila backup from a zip file. |
...
| `hermes acp` | Run Aquila as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run Aquila as an MCP server. |
| `hermes plugins` | Manage Aquila Agent plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated Aquila instances. |
...
| `hermes uninstall` | Remove Aquila from the system. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Aquila itself. |
...
hermes chat --safe-mode -q "Is this bug mine or Aquila'?"
```
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Aquila chat session.
...
**`hermes model`** (run from your terminal, outside any Aquila session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Aquila chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Aquila session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `--name NAME` | `Aquila` | Bot display name in Slack. |
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into Aquila) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Aquila venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hermes/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Aquila Python venv. |
...
hermes auth spotify                                      # Authenticate Aquila with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire Aquila setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Aquila version, release date, git commit hash |
...
The report includes system info (OS, Python version, Aquila version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
Create a zip archive of your Aquila configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Aquila is running (WAL-mode safe).
...
Restore a previously created Aquila backup into your Aquila home directory. All files in the archive overwrite existing files in your Aquila home; `--force` only skips the confirmation prompt that fires when the target already has a Aquila installation.
...
View, tail, and filter Aquila log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
Aquila uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how Aquila layers
...
- `--source well-known` lets you point Aquila at a site exposing `/.well-known/skills/index.json`.
...
`hermes moa configure` reuses Aquila' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
Starts Aquila as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Aquila as an MCP server.
...
| `serve [-v\|--verbose]` | Run Aquila as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Aquila](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Aquila Plugin](../developer-guide/plugins/index.md).
...
right now without waiting for the next Aquila update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Aquila shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to Aquila. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Aquila files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Aquila equivalents or **archived** for manual review.
...
Manage profiles — multiple isolated Aquila instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Aquila commands, subcommands, and profile names.
...
`hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Aquila may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Gateway restart.** After a successful update, Aquila attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, Aquila makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
...
- **Legacy `hermes.service` warning.** If Aquila detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes postinstall` | Internal bootstrap. Runs once after the install script provisions Aquila (or after `hermes update`) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `hermes setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `hermes uninstall [--full] [--gui] [--yes]` | Remove Aquila, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |
