---
sidebar_position: 5
title: "Using Aquila as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using Aquila as a Python Library

Aquila isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install Aquila directly from the repository:
...
:::tip
The same environment variables used by the CLI are required when using Aquila as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Aquila is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding Aquila in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, Aquila includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
