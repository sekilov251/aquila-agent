---
sidebar_position: 7
title: "Use SOUL.md with Aquila"
description: "How to use SOUL.md to shape Aquila Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Aquila

`SOUL.md` is the **primary identity** for your Aquila instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Aquila to feel like the same assistant every time you talk to it — or if you want to replace the Aquila persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Aquila should be
- what Aquila should avoid stylistically
- how Aquila should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Aquila is and how Aquila speaks
...
Aquila now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Aquila with a custom home directory, it becomes:
...
Aquila automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Aquila does not overwrite it
- if the file exists but is empty, Aquila adds nothing from it to the prompt

## How Aquila uses it

When Aquila starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Aquila feels.
...
Aquila already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Aquila should sound.

# Avoid
What Aquila should not do.

# Defaults
How Aquila should behave when ambiguity appears.
```
...
Then restart Aquila or start a new session.
...
### I edited SOUL.md but Aquila still sounds the same
...
### Aquila is ignoring parts of my SOUL.md
