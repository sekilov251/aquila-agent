...
2. **Aquila spins up** a fresh agent session with your prompt
...
Aquila will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Aquila Agent
```
...
Before creating cron jobs, ensure Aquila has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Aquila what you want:
...
Aquila will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Aquila to delegate each topic to a sub-agent:
...
Aquila will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
