...
messaging platform Aquila is already configured for. Think of it as a
...
Any platform Aquila ships adapters for works as a target:
...
When stdin is a TTY (no pipe), Aquila does **not** wait for input — you'll
...
Because `hermes send` reuses your Aquila config, the same script works on
any host where Aquila is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses Aquila creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
