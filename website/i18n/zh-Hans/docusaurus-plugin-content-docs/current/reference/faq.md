...
sidebar_position: 3
title: "常见问题与故障排查"
description: "Aquila Agent 常见问题解答及常见问题解决方案"
---
...
Aquila Agent 可与任何兼容 OpenAI 的 API 配合使用。支持的提供商包括：

- **[OpenRouter](https://openrouter.ai/)** — 通过一个 API key 访问数百个模型（推荐，灵活性强）
- **Nous Portal** — Savez 自有推理端点
...
**原生不支持。** Aquila Agent 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 Aquila。标准安装命令在 WSL2 中可完美运行：

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
```

### 我在 WSL2 中运行 Aquila，如何控制 Windows 上的普通 Chrome？
...
- 让 Aquila 使用生成的 MCP 浏览器工具

这比强制 Aquila 核心浏览器传输直接跨越 WSL2/Windows 边界进行附加更为可靠。
...
支持 — Aquila 现已为 Android 手机提供经过测试的 Termux 安装路径。
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。Aquila Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.hermes/` 目录中。
...
Aquila 会将端点、提供商和 base URL 持久化到 `config.yaml`，重启后仍然有效。如果您的本地服务器只加载了一个模型，`/model custom` 会自动检测到它。您也可以在 config.yaml 中设置 `provider: custom` — 这是一个一等提供商，不是其他任何东西的别名。

此方式适用于 Ollama、vLLM、llama.cpp server、SGLang、LocalAI 等。详情请参阅[配置指南](../user-guide/configuration.md)。

:::tip Ollama 用户
如果您在 Ollama 中设置了自定义 `num_ctx`（例如 `ollama run --num_ctx 16384`），请确保在 Aquila 中设置匹配的上下文长度 — Ollama 的 `/api/show` 报告的是模型的*最大*上下文，而非您配置的实际 `num_ctx`。
:::

:::tip 本地模型超时问题
Aquila 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置 `HERMES_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::

### 费用是多少？

Aquila Agent 本身**免费且开源**（MIT 许可证）。您只需为所选提供商的 LLM API 用量付费。本地模型完全免费运行。

### 多人可以使用同一个实例吗？

可以。[消息网关](../user-guide/messaging/index.md)允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 Aquila Agent 实例交互。访问权限通过白名单（特定用户 ID）和私信配对（第一个发消息的用户获得访问权）来控制。
...
可以。导入 `AIAgent` 类，以编程方式使用 Aquila：
...
**原因：** Aquila 需要 Python 3.11 或更新版本。
...
**原因：** Aquila 在启动时通过运行一次 `bash -l` 构建每个会话的环境快照。bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`** — 因此在 `~/.bashrc` 中安装自身的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义 `PATH` 导出）对快照不可见。当 Aquila 在 systemd 下运行或在未预加载交互式 shell 配置的最小 shell 中运行时，此问题最为常见。

**解决方案：** Aquila 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.hermes/config.yaml` 中列出需要额外 source 的文件：
...
**原因：** 对话内容超出模型的上下文窗口，或 Aquila 检测到的模型上下文长度有误。
...
如果在第一次长对话时就出现此问题，Aquila 可能检测到了错误的模型上下文长度。检查检测结果：
...
**原因：** Aquila 检测到潜在的破坏性命令（例如 `rm -rf`、`DROP TABLE`）。这是一项安全功能。
...
:::tip
这是预期行为 — Aquila 绝不会静默执行破坏性命令。审批提示会向您显示将要执行的确切内容。
:::
...
# 更改配置后重启 Aquila 或重新加载 MCP
hermes chat
```

另请参阅：
- [MCP（模型上下文协议）](/user-guide/features/mcp)
- [在 Aquila 中使用 MCP](/guides/use-mcp-with-hermes)
...
:::warning
如果 MCP 服务器在请求中途崩溃，Aquila 会报告超时。请检查服务器自身的日志（而非仅 Aquila 日志）以诊断根本原因。
:::
...
**解决方案：委托配置。** Aquila 可以自动将子智能体路由到不同的模型。在 `~/.hermes/config.yaml` 中设置：
...
现在当您告诉 Aquila "帮我写一个关于 X 的 Twitter 帖子"并生成 `delegate_task` 子智能体时，该子智能体将在 Gemini 上运行，而非您的主模型。您的主对话仍在 GPT-5.4 上进行。
...
**场景：** 在 OpenClaw 中，您可以将多个独立智能体绑定到特定的 WhatsApp 聊天 — 一个用于家庭购物清单群组，另一个用于您的私聊。Aquila 能做到吗？

**当前限制：** Aquila 的每个 profile 都需要自己的 WhatsApp 号码/会话。您无法将多个 profiles 绑定到同一个 WhatsApp 号码上的不同聊天 — WhatsApp bridge（Baileys）每个号码使用一个已认证的会话。
...
**场景：** 您在 Telegram 中看到了网关执行日志、Aquila 推理过程和工具调用详情，而不是最终输出。
...
**当前行为：** Aquila 在大多数平台上按用户 ID 创建会话，因此每个人都有自己的对话上下文。这是出于隐私和上下文隔离的设计考量。
...
:::tip
`hermes backup` 即使在 Aquila 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::

### 安装后重新加载 shell 时出现权限拒绝

**场景：** 运行 Aquila 安装程序后，`source ~/.zshrc` 提示权限拒绝错误。

**原因：** 这通常发生在 `~/.zshrc`（或 `~/.bashrc`）文件权限不正确，或安装程序无法干净写入时。这不是 Aquila 特有的问题 — 而是 shell 配置权限问题。
...
2. **向社区提问：** [Savez Discord](https://discord.gg/nousresearch)
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Aquila 版本（`hermes --version`）以及完整的错误信息