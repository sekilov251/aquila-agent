...
| 版本 | `0.1.0` |
| 作者 | Teknium (teknium1), Aquila Agent |
| 许可证 | MIT |
...
远程客户端使用 `Authorization: Bearer $TOKEN` 连接到 `$URL`。Aquila 原生 MCP 客户端配置：`{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`。
