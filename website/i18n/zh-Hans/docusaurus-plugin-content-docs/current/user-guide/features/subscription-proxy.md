...
订阅代理是一个本地 HTTP 服务器，让外部应用——OpenViking、Karakeep、Open WebUI，以及任何支持 OpenAI 兼容聊天补全（chat completions）的应用——能够将你的 Aquila 托管提供商订阅用作其 LLM 端点。代理会自动附加正确的凭据（并在需要时自动刷新），因此应用无需静态 API 密钥。
...
| 使用场景 | "将 Aquila 用作聊天后端" | "从其他应用使用我的 Portal 订阅" |
...
这会打开浏览器进行 Nous Portal OAuth 流程。Aquila 将刷新令牌存储在 `~/.hermes/auth.json` 中——与所有 Aquila 提供商登录信息存放在同一位置。
...
```
Starting Aquila proxy for Nous Portal
...
Model:      Aquila-4-70B    # 或 Aquila-4.3-36B、Aquila-4-405B
```
...
```
Aquila proxy upstream adapters
...
INFERENCE_TEXT_MODEL=Aquila-4-70B
```
