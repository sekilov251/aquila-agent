...
Aquila Agent 内置两个可供模型调用的网页工具，由多个提供商支持：
...
摘要保留引用、代码块和关键事实的原始格式——它是内容压缩器，而非改写器。如果摘要失败或超时，Aquila 会回退到原始内容的前约 5 000 字符，而非返回无用的错误信息。
...
SearXNG 是一个注重隐私的开源元搜索引擎，聚合来自 70 多个搜索引擎的结果。**无需 API 密钥**——只需将 Aquila 指向一个运行中的 SearXNG 实例。
...
使用此配置，Aquila 对所有搜索查询使用 SearXNG，对 URL 提取使用 Firecrawl——将免费搜索与高质量提取相结合。
...
如果未显式配置后端，Aquila 根据已设置的凭证选择第一个可用的后端：
