...
4. 填写以下内容：
   - **Name：**`Aquila Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Aquila app
New-CsApplicationAccessPolicy `
  -Identity "Aquila-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Aquila meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Aquila-Meeting-Pipeline-Policy" `
...
Aquila 内置了 Graph 身份验证冒烟测试。在 Aquila 安装目录下执行：
