...
Optional Aquila observability plugin that maps Aquila observer hooks to
...
not replace Aquila Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Aquila emit NeMo Relay lifecycle events for
the work Aquila already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Aquila Agent can:

- Preserve Aquila execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
`HERMES_HOME` is the Aquila profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Aquila uses the user's default home, usually `~/.hermes`. For isolated smoke
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Aquila does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Aquila keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Aquila routes LLM and tool
...
Aquila feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Aquila-owned
...
with this Aquila-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Aquila rejects
...
Aquila activates these plugins before registering its managed LLM and tool
...
activation API, Aquila logs an actionable warning and continues with the
...
For the full generic Aquila middleware contract, see
...
This run starts a parent Aquila session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Aquila Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Aquila Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Aquila dispatches the read-only tools as one batch, and NeMo Relay
...
- Aquila sessions map to `agent` scopes.
- Aquila API request hooks map to `llm` scope start/end events.
- Aquila tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Aquila execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Aquila routes execution
through these boundaries:

```text
Aquila provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Aquila provider adapter next_call(...)

Aquila tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Aquila tool dispatcher next_call(...)
```
...
middleware for a local Aquila run. This path requires a NeMo Relay runtime that
...
agent_name = "Aquila Middleware E2E"
agent_version = "local"
