...
This plugin connects Aquila Agent to iMessage (and other Spectrum
...
TypeScript-only, Aquila runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Aquila-Sidecar-Token`.
...
2. **Find or create** the `Aquila Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Aquila channel,
