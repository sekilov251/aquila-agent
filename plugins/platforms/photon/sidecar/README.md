...
Small Node helper that bridges Aquila Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Aquila is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Aquila-Sidecar-Token`)
...
The Aquila plugin's `hermes photon setup` command runs `npm install`
here automatically.
